...
document
  .getElementById("list_file_upload")
...
function addField_v2() {
  ...
  const added_field_description = document.getElementById(
  ...
  if (added_field_type === "slt") {
    ...
    input.setAttribute("name_in_report", added_field_name);
  ...
  // multi select
    ...
    input_field.setAttribute("name_in_report", added_field_name);
  ...
  for (let i = 0; i < formElements.length; i++) {
    ...
    if (element.localName === "ul") {
      ...
      const field = {
        id: element.id,
        value: dropdownItems,
        name: element.getAttribute("name_in_report"),
      };
      addedFields.push(field);
    } else if (element.localName === "select") {
      const field = {
...
function getTemplate() {
  ...
  // Loop through all the form elements and get the attributes
  const formElements_for_template = form_for_template.querySelectorAll(
    "input, select, textarea, ul"
  ); // Select the relevant elements
...
function updateSelectBoxStyle() {
  const selectBox = document.getElementById("risk_level_input");
  const value = selectBox.value.toLowerCase(); // Get the selected value
  console.log(value)
  // Remove existing color classes
  selectBox.classList.remove("bg-green", "bg-yellow", "bg-red");

  // Apply the appropriate class based on the value
  if (value === "low") {
    selectBox.classList.add("bg-green");
    console.log('green')
  } else if (value === "medium") {
    selectBox.classList.add("bg-yellow");
  } else if (value === "high") {
    selectBox.classList.add("bg-red");
  }
}
...
selectBox = document.getElementById("risk_level_input")
if(selectBox){
  document.addEventListener("DOMContentLoaded", updateSelectBoxStyle);
  selectBox.addEventListener("change", updateSelectBoxStyle);
  selectBox.addEventListener("select", updateSelectBoxStyle);
  selectBox.addEventListener("click", updateSelectBoxStyle);
  selectBox.addEventListener("keydown", updateSelectBoxStyle);
}
